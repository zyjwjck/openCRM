...
<svelte:head>
  <title>{t('cases')} - openCRM</title>
</svelte:head>

<PageHeader title={t('cases')} subtitle={`${filteredCases.length} of ${casesData.length} ${t('cases')}`}>
  {#snippet actions()}
    ...
    >
      {#snippet emptyState()}
        <div class="flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 class="text-[var(--text-primary)] text-lg font-medium">{t('noCasesFound')}</h3>
...
<CrmDrawer
  ...
  titleKey="subject"
  titlePlaceholder={t('caseTitle')}
  headerLabel={drawer.mode === 'create' ? t('newCase') : t('case')}
  ...
  {#snippet footerActions()}
    {#if drawer.mode !== 'create' && drawer.selected}
      {#if drawerFormData.status === 'Closed'}
        <Button variant="outline" onclick={handleReopen} disabled={isSubmitting}>{t('reopen')}</Button>
      {:else}
        <Button variant="outline" onclick={handleClose} disabled={isSubmitting}>{t('closeCase')}</Button>
      {/if}
    {/if}
    ...
    {#if isDrawerDirty || drawer.mode === 'create'}
      <Button onclick={handleSave} disabled={isSubmitting}>
        {#if isSubmitting}
          ...
          {drawer.mode === 'create' ? t('creating') : t('saving')}
        {:else}
          {drawer.mode === 'create' ? t('createCase') : t('saveChanges')}
        {/if}
      </Button>
    {/if}
  {/snippet}
