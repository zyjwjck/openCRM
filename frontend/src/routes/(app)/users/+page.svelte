<script>
  ...
  function handleTeamSubmit(formData) {
    ...
    form.method = 'POST';
    form.action = formData.teamId ? '/users?/update_team' : '/users?/create_team';
  ...
  function handleActivateUser(userId) {
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = '/users?/activate_user';
    form.style.display = 'none';
