...
<svelte:head>
  <title>{t('opportunities')} - openCRM</title>
</svelte:head>

<PageHeader title={t('opportunities')} subtitle={`${t('pipeline')}: ${formatCurrency(stats.pipeline)}`}>
  {#snippet actions()}
    <div class="flex items-center gap-2">
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end" class="w-48">
          <DropdownMenu.Label>{t('toggleColumns')}</DropdownMenu.Label>
...
<div class="flex-1">
  ...
  {#if filteredOpportunities.length === 0}
    <div class="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 class="text-[var(--text-primary)] text-lg font-medium">{t('noOpportunitiesFound')}</h3>
      <p class="text-[var(--text-secondary)] mt-1 text-sm">{t('tryAdjustingFilters')}</p>
    </div>
  {:else}
    <CrmTable
...
<CrmDrawer
  ...
  titleKey="name"
  titlePlaceholder={t('opportunityName')}
  headerLabel={t('opportunity')}
  mode={drawerMode}
  loading={isLoading}
    ...
    {#if drawerMode !== 'create' && selectedRow}
      <div class="mb-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p class="text-xs text-[var(--text-tertiary)]">{t('account')}</p>
            <p class="font-medium text-[var(--text-primary)]">
              {selectedRow.account?.name || t('notAssigned')}
            </p>
          </div>
          {#if selectedRow.createdAt}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">{t('created')}</p>
              <p class="font-medium text-[var(--text-primary)]">
          ...
          {#if isClosed && selectedRow.closedBy}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">{t('closedBy')}</p>
              <p class="font-medium text-[var(--text-primary)]">
                {selectedRow.closedBy?.name || t('system')}
              </p>
            </div>
          {/if}
          {#if isClosed && selectedRow.closedOn}
            <div>
              <p class="text-xs text-[var(--text-tertiary)]">{t('closedOn')}</p>
              <p class="font-medium text-[var(--text-primary)]">
                {formatRelativeDate(selectedRow.closedOn)}
              </p>
            </div>
          {/if}
        </div>
      ...
      <div class="mt-6 border-t border-[var(--border-default)] pt-4">
        ...
        {:else}
          <p class="mb-4 text-sm text-[var(--text-secondary)]">{t('noProductsAddedYet')}</p>
        {/if}
        ...
        {#if !isClosed}
          ...
          >
            <p class="text-xs font-medium text-[var(--text-secondary)]">{t('addProduct')}</p>
  ...
  {#snippet footerActions()}
    {#if drawerMode === 'create'}
      <Button onclick={handleDrawerSave} disabled={isLoading || !drawerFormData.name?.trim()}>
        {isLoading ? t('creating') : t('createOpportunity')}
      </Button>
    {:else}
      {#if !isClosed}
        <Button
      ...
      {#if canCreateInvoice}
        <AlertDialog.Root bind:open={createInvoiceDialogOpen}>
          ...
          <AlertDialog.Content>
            <AlertDialog.Header>
              <AlertDialog.Title>{t('createInvoice')}</AlertDialog.Title>
              <AlertDialog.Description>
                {t('thisWillCreateANewInvoice')} {lineItems.length} {lineItems.length === 1 ? t('product') : t('products')} {t('totaling')} {formatCurrency(
                  selectedRow?.lineItemsTotal || 0,
                  selectedRow?.currency || 'USD'
                )}.
              </AlertDialog.Description>
            ...
            <AlertDialog.Footer>
              ...
              <AlertDialog.Action onclick={confirmCreateInvoice}>{t('createInvoice')}</AlertDialog.Action>
            </AlertDialog.Footer>
          </AlertDialog.Content>
        </AlertDialog.Root>
      {/if}
