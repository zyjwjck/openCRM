<script lang="ts">
	...
	const createCopyCodeBlockButton = () => {
		...
		blocks.forEach((block) => {
			...
			if (navigator.clipboard && block.childNodes.length < 2 && block.id !== "user-message") {
				...
				button.textContent = t('copyCode', language);
		...
		async function copyCode(block, button) {
			...
			// visual feedback that task is completed
				button.innerText = t('copied', language);

				setTimeout(() => {
					button.innerText = t('copyCode', language);
				}, 1000);
		}
	};
	...
	const copyToClipboard = (text) => {
		...
		navigator.clipboard.writeText(text).then(
			function () {
					console.log(t('copySuccess', language));
					toast.success(t('copySuccess', language));
				},
			function (err) {
		...
		if (saved) {
			try {
				...
				if (chat) {
					...
					localStorage.setItem('chats', JSON.stringify(chatsList));
					// Reload chats from local storage
						...
						{:else}
							...
							>
								{#if message.role == "user"}
										...
										{:else}
									<div class="w-full text-right">
							<div id="user-message" class="text-right whitespace-pre-wrap break-words bg-gray-100 dark:bg-gray-800 p-3 rounded-lg">{message.content}</div>
