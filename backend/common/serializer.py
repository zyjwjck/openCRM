...
def find_urls(string):
    # website_regex = "^((http|https)://)?([A-Za-z0-9.-]+\.[A-Za-z]{2,63})?$"  # (http(s)://)example.com or example.com
    # website_regex = "^https?://([A-Za-z0-9.-]+\.[A-Za-z]{2,63})?$"  # (http(s)://)example.com
    # http(s)://example.com
...
class APISettingsSerializer(serializers.ModelSerializer):
    ...
    def validate_website(self, website):
        ...
        if not len(find_urls(website)) > 0:
            raise serializers.ValidationError(
                "Please provide a valid URL with schema and without trailing slash - Example: http://example.com"
            )
        return website
